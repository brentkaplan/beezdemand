# Test Koffarnus equation support in fit_demand_mixed()

test_that("fit_demand_mixed() accepts equation_form = 'koff'", {
  data(apt, package = "beezdemand")

  # Should run without error with koff equation
  expect_no_error({
...
test_that("koff equation with log10 param_space produces valid results", {
  data(apt, package = "beezdemand")
  ...
  expect_equal(fit$formula_details$equation_form_selected, "koff")
...
test_that("koff equation with natural param_space produces valid results", {
  data(apt, package = "beezdemand")
...
test_that("koff equation calculates k automatically when not provided", {
  data(apt, package = "beezdemand")
...
test_that("koff equation formula is correctly constructed", {
  data(apt, package = "beezdemand")
  ...
  # Check formula contains the expected koff pattern
...
test_that("predict works with koff equation models", {
  data(apt, package = "beezdemand")
...
test_that("summary works with koff equation models", {
  data(apt, package = "beezdemand")
...
test_that("koff equation works with factors", {
  data(apt, package = "beezdemand")
