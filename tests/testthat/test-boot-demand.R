...
test_that("boot_demand warns exactly once when hessian_pd is FALSE (dedup through draws->vcov)", {
  ...
  # boot_demand() now propagates as a classed error. The hessian_pd warning is
  # still raised exactly once on the way, which is what this test pins.
  ...
  conds <- .capture_warning_conditions(
    res <- tryCatch(
      boot_demand(fit, statistics = "Pmax", R = 100, seed = 1),
