...
The production `k` argument (`"share"`, `"range"`, `"fit"`, `"ind"`, or
a plain number) means different things and cannot simply be re-passed
unchanged to a rescue call that fits a single subject at a time:
...
- `"share"`:
  ...
  only one dataset!") on a single-subject dataset, so every rescue
  attempt would silently fail via
  [`try()`](https://rdrr.io/r/base/try.html), never actually rescuing
  anything.
