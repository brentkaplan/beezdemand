...
- newdata:
  ...
  `x_var`. If `NULL`, uses the unique observed prices when available.
...
A tibble containing the original `newdata` columns, plus `.fitted` and,
when requested, `.se.fit` and `.lower`/`.upper`. If `newdata` does not
include an id column, predictions are returned for all subjects (cross
product of `newdata` × subjects) unless `k` is subject-specific
(`k = "ind"`).
