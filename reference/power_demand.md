...
deviation in condition 1 is independent of their deviation in condition
2, and `rho_bd` (the within-condition Q0/alpha correlation) is pinned at
0. No subject-level effect is shared across conditions, so the
within-subject power estimate describes two conditions with independent
subject effects. In the usual repeated-measures design each subject's
deviation carries over and the condition contrast is paired; that design
is not what is simulated here. When the carried-over deviations are
positively correlated across conditions (the common case) pairing
reduces the contrast variance, so the power reported here is a
conservative benchmark for such a design; a negative cross-condition
correlation would make it optimistic instead. The default
`random_effects` (per-condition effects) matches the simulated
...
exactly, as the within-subject default matches its own. In both designs
the additive-Gaussian residual likelihood is a *working model* for the
simulator's multiplicative-lognormal errors (closest at small
`sigma_e`). Type I error is calibrated by the test suite for both
designs.
