/// @file HurdleCrossPrice2RE.h
/// Two-Part Mixed Effects Hurdle Cross-Price Demand Model - TWO RANDOM EFFECTS
/// =============================================================================
///
/// MODEL STRUCTURE:
/// Part I (Binary):  logit(pi_ij) = beta0 + beta1*log(x+epsilon) + a_i
/// Part II (Continuous): log(Q_ij) = (logQalone + b_i) + I*exp(-beta*x) + e_ij
///   NOTE: I is FIXED (no c_i random effect)
...
template <class Type>
Type HurdleCrossPrice2RE(objective_function<Type>* obj) {
  // DATA
  ...
  for (int i = 0; i < n; i++) {
    ...
    // log(Q) = (logQalone + b_i) + I * exp(-beta * x)
    Type mu_ij = (logQalone + b_i) + I * exp(-beta_param * x(i));
