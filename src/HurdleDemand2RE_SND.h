...
/// Part II: log(Q_ij) = (log_q0 + b_i) - alpha * Q0_i * price + e_ij
///   where Q0_i = exp(log_q0 + b_i)
///   and alpha = exp(log_alpha)
...
template <class Type>
Type HurdleDemand2RE_SND(objective_function<Type>* obj) {
  // DATA
  DATA_VECTOR(price);
  DATA_VECTOR(logQ);
  DATA_IVECTOR(delta);
  ...
  for (int i = 0; i < n; i++) {
    ...
    Type mu_ij = (log_q0 + b_i) - alpha * Q0_i * price(i);
