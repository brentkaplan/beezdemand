...
| Group comparisons / random effects (modern TMB backend) | `fit_demand_tmb()` | [TMB mixed-effects](https://brentkaplan.github.io/beezdemand/articles/tmb-mixed-effects.html) |
...
fixed-effect NLS (`fit_demand_fixed()`), nonlinear mixed-effects via
`nlme` (`fit_demand_mixed()`), mixed-effects via Template Model Builder
(`fit_demand_tmb()`, the recommended backend for new mixed-effects
...
`fit_demand_tmb()` fits the same hierarchical demand models as
`fit_demand_mixed()` but is built on [Template Model Builder
(TMB)](https://github.com/kaskr/adcomp), which uses exact automatic
...
# Hursh & Silberberg exponential model with random intercepts on Q0 and alpha
...
# With a between-subjects factor and post-hoc comparisons
data(apt_full, package = "beezdemand")
fit_tmb_grp <- fit_demand_tmb(apt_full, equation = "exponential", factors = "gender")
get_demand_param_emms(fit_tmb_grp, param = "alpha")
```

See the [TMB mixed-effects
vignette](https://brentkaplan.github.io/beezdemand/articles/tmb-mixed-effects.html)
(`vignette("tmb-mixed-effects")`) for a full walkthrough, and the
[advanced random-effects
guide](https://brentkaplan.github.io/beezdemand/articles/tmb-advanced-random-effects.html)
for factor-expanded and multi-block covariance structures.
