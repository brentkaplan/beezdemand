...
- test:
  ...
  Character; type of statistical test. One of: - \`"auto"\` (default):
  Use LRT if models are nested and comparable, otherwise IC-only. -
  \`"lrt"\`: Force likelihood ratio test (requires nested models from
  same backend). - \`"none"\`: Only report information criteria, no
  p-value.
...
hurdle \| hurdle \| Yes (if nested) \| \| nlme \| nlme \| Yes (if
nested) \| \| fixed \| fixed \| No (no likelihood) \| \| hurdle \| nlme
\| No \| \| hurdle \| fixed \| No \| \| nlme \| fixed \| No \|

## Statistical Notes

The likelihood ratio test (LRT) assumes that:

1.  The models are \*\*nested\*\* (the reduced model is a special case
    of the full model obtained by constraining parameters).
...
\*\*Important caveat for mixed-effects models:\*\* When variance
components are tested at the boundary (e.g., testing whether a random
effect variance is zero), the standard chi-square distribution is not
...
This function does \*\*not\*\* automatically verify that models are
nested. Users should ensure models are properly nested before
interpreting LRT p-values.
