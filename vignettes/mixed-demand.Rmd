...
The loss surface and profile plots visualize the optimization landscape. The
NLME versions support two modes:

- `type = "ssr"` (default): sum-of-squared residuals on aggregated price-level
  means (a quick visual check of the fixed-effects surface)
- `type = "marginal"`: linearized marginal negative log-likelihood that
  integrates over the random-effects variance. This mode is more appropriate for
  mixed models and is preferred for assessing identifiability near the MLE.

```{r loss-ssr, fig.cap = "SSR loss surface on aggregated means."}
plot_loss_surface(fit_apt_zben)
```

```{r loss-marginal, fig.cap = "Marginal NLL surface accounting for random-effects variance."}
plot_loss_surface(fit_apt_zben, type = "marginal")
```

Profile plots show 1D slices (one parameter varied, the other fixed at the MLE):

```{r profile-ssr, fig.cap = "SSR profile plots for Q0 and alpha."}
plot_loss_profile(fit_apt_zben)
```

```{r profile-marginal, fig.cap = "Marginal NLL profile plots for Q0 and alpha."}
...
We now model $Q_{0}$ and $\alpha$ as varying by dose for Alfentanil.
...
We use `fit_one_factor_dose` (modeling demand for Alfentanil
...
The beezdemand package provides tools for fitting nonlinear mixed-effects
demand models and interpreting their parameters. Parameterizing $Q_{0}$ and
$\alpha$ on the log10 scale improves numerical stability, and the helper
functions back-transform estimates to the natural scale for interpretation.
