...
- **Model comparison and overlay** with `compare_models()`, `anova()`, and
  `plot_demand_overlay()`
...
Note that with complex models and small sample sizes, convergence can fail.
With `start_value_method = "pooled_nls"`, fits often converge where the heuristic start values do not.
...
This is useful when you want fine-grained distinctions for one
...
We use `fit_two_factors_add` (if it converged: y_ll4 ~ drug + dose) and
...
This function returns estimated marginal means for $Q_{0}$ and $\alpha$ on both the log10 and natural scales, and optionally the essential value (EV).
...
Individual-level predicted coefficients come from the
...
Use `anova()` to formally compare nested NLME models via likelihood ratio test.
For example, compare a one-factor model against a no-factor (intercept-only)
baseline:
...
The cross-tier `compare_models()` function also accepts NLME objects and
returns the same output format as for TMB and hurdle fits:

```{r model-comparison-compare}
compare_models(fit_no_factors_alf, fit_one_factor_dose)
```

### Overlaying Demand Curves

`plot_demand_overlay()` superimposes population demand curves from multiple
models, which shows how different specifications shift the predicted
curve:

```{r viz-overlay-nlme, fig.cap = "Demand curves from intercept-only and dose-factor models overlaid."}
plot_demand_overlay(
...
`plot_model_comparison()` compares parameter distributions side by side:

```{r viz-model-comparison-nlme, fig.cap = "Side-by-side parameter estimates from two NLME models."}
